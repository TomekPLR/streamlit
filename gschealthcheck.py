...
exclude_from_output = [ 'Number of Good URLs (Mobile)','Number of Need Improvement URLs (Mobile)','Number of Poor URLs (Mobile)','Number of pages not indexed','Number of Indexed pages'  ]  # Add field names you want to exclude from output
...
group_descriptions = { 
    'Core Web Vitals report': 'To access the report scroll down and click on Core Web Vitals. Alternatively, type this URL: <a href="https://search.google.com/search-console/core-web-vitals">https://search.google.com/search-console/core-web-vitals</a>',
    'Crawl stats report': 'To access this report, you need to scroll down, click on "Settings" then "Crawl Stats". Alternatively, you can type this URL: <a href="https://search.google.com/search-console/settings/crawl-stats">https://search.google.com/search-console/settings/crawl-stats</a>',
    'Indexing report': 'To enter data about indexing, go to Indexing -> Pages. Alternatively, click visit this URL: <a href="https://search.google.com/search-console/index">https://search.google.com/search-console/index</a>',
}
    ...
    with st.expander("Detailed Results"):
        for field, value in user_values.items():
            if field not in exclude_from_output:  # Exclude fields from detailed results
                ...
                st.markdown(f"<h3 style='text-align: center;'>{result_message} {field}: {value}</h3>", unsafe_allow_html=True)
                ...
                st.markdown(f"<h5 style='text-align: center;'>{'higher' if better else 'lower'} than median = {median_value}</h4>", unsafe_allow_html=True)
