...
def main_analysis():
    ...
    if submit_button:
        ...
        differences_with_user_title_summary = query_chatgpt(f"Here's the Google search results: {st.session_state['stored_results']}. Give summary of what's the difference between focus of top 3 results (includes 1,2,3) and the title proposed by the user: '{st.session_state['user_title']}'. At the end, provide me with similarity score from 0 to 10")
        st.subheader("📝 Differences Summary with User Title")
        st.write(differences_with_user_title_summary)

        relevancy_score = query_chatgpt(f"How well is the '{st.session_state['user_title']}' proposed by the user relevant to the '{st.session_state['stored_query']}'? Give me score 0 to 10.")
