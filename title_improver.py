...
def main_analysis():
    ...
    if submit_button:
        ...
        differences_summary = query_chatgpt(f"Here's the Google search results: {st.session_state['stored_results']}. Give summary of what's the difference between focus of top 3 results (includes 1,2,3) and the places 7-10. Be harsh")
        st.subheader("✏️ Differences Summary")
        st.write(differences_summary)

        differences_with_user_title_summary = query_chatgpt(f"Here's the Google search results: {st.session_state['stored_results']}. Give summary of what's the difference between focus of top 3 results (includes 1,2,3) and the title proposed by the user: '{st.session_state['user_title']}'. At the end, provide me with similarity score from 0 to 10. Be harsh")
        st.subheader("📝 Differences Summary with User Title")
        st.write(differences_with_user_title_summary)

        relevancy_score = query_chatgpt(f"How well is the '{st.session_state['user_title']}' proposed by the user relevant to the '{st.session_state['stored_query']}'? Give me score 0 to 10. Be harsh")
        st.subheader("🎯 Relevancy Score <title, query>")
        st.write(relevancy_score)

        # New analysis on the article for signs of waffle content
        waffle_content_analysis = query_chatgpt(f"I have an article: {st.session_state['user_article']}\n\nLook for signs of waffle content, for instance: Repetition, Generalization, Lack of Depth, Limited Insights, Absence of Examples or Case Studies. Then judge how useful it's for the users typing in Google: '{st.session_state['stored_query']}'. Give me just summary, I don't need detailed problems in each category.  Give the general score at the end. Then give the score for those looking for general guidance followed by the score for those how useful is it for those in need of a comprehensive understanding or to take effective action? Be harsh")
